...
if (!Number(args[0])) return m.reply(`⚔️🧐 Debes depositar una cantidad válida.\n> Ejemplo 1 » *#d 25000*\n> Ejemplo 2 » *#d all*`)
let count = parseInt(args[0])
if (!user.coin) return m.reply🧐 No tienes suficientes *monedas* para depositarlas.`)
if (user.coin < count) return m.reply👀 Solo tienes *${user.coin} monedas*`)
user.coin -= count * 1
user.bank += count * 1
await m.reply(`${emoji} Depositaste *${count} monedas 🪙* en el banco, ya no podran robarte.`)}
