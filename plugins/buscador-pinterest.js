...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    return conn.reply(m.chat, `🧐 *inyecta un texto para buscar tu imagen*  
Escribe algo para que busque en Pinterest, imbécil de humano~ 🧐`, m);
  }

  let query = text + " hd";
  await m.react("🔎");
  conn.reply(m.chat, ` *espera soy lento 🦥~*  
enviando su imagen...~ 👀`, m);
