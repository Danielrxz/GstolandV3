...
const handler = async (m, { text, conn, args }) => {
  if (!args[0]) {
    return conn.reply(m.chat, `🧐 ingresa el enlace de facebook.`, m)
  }
  ...
  let video = data.url;
  try {
    await conn.sendMessage(m.chat, { video: { url: video }, caption: `Video de Facebook descargar por Gatoland by Danielrxz.`, fileName: 'fb.mp4', mimetype: 'video/mp4' }, { quoted: m })
    await m.react(done);
  } catch (e) {
